<?php
$theme_options = get_option( 'themeworks_options' );

/**
 * Register email in MailChimp
 */
function themeworks_mailchimp_register( $list, $email, $merge_vars ) {
    global $theme_options;
    // Process Subscription
    if ( strlen ( trim ( $theme_options['mailchimp_api_key'] ) ) > 0 ) {
        $api = new MCAPI( $theme_options['mailchimp_api_key'] );
        if ( $api->listSubscribe( $list, $email, $merge_vars ) === true ) {
            return true;
        }
    }
    return false;
 ...
 */
function themeworks_signup_processor() {
    global $theme_options;
    $list       = $theme_options['mailchimp_list'];
    $email      = $_POST['email'];
    ...
    if ( themeworks_mailchimp_register( $list, $email, $merge_vars ) ) {
        echo "success";
    } else {
        echo "fail";
    }
...
add_action( 'wp_ajax_themeworks_signup', 'themeworks_signup_processor' );
add_action( 'wp_ajax_nopriv_themeworks_signup', 'themeworks_signup_processor' );

/**
 * MailChimp lists
 */
function themeworks_mailchimp_get_lists() {
    global $theme_options;
    // Get lists
    if ( ! empty ( $theme_options['mailchimp_api_key'] ) && strlen ( trim ( $theme_options['mailchimp_api_key'] ) ) > 0 ) {
        ...
        if ( $list_data ) {
            foreach( $list_data['data'] as $key => $list ) {
                ...
                $lists[$key]['name']    = $list['id'];
                $lists[$key]['title']   = $list['name'];
                $lists[$key]['members'] = $members['total'];
            }
        }

        // Add our default value of "none" to be the first item in our array
        array_unshift( $lists, array( 'name' => 0, 'title' => __( 'None', 'themeworks' ) ) );
        return $lists;
    }
    return array();
}
