...
( function( api ) {
    var cssTemplate = wp.template( 'themeworks-color-scheme' ),
        colorSchemeKeys = [
            'background_color',
            'accent_color',
            'dark_color'
        ...
        colorSettings = [
            'background_color',
            'themeworks_options[accent_color]',
            'themeworks_options[dark_color]'
        ];

    api.controlConstructor.select = api.Control.extend( {
        ready: function() {
            if ( 'themeworks_color' === this.id ) {
                this.setting.bind( 'change', function( value ) {
                    ...
                    api( 'themeworks_options[accent_color]' ).set( colorScheme[value].colors[1] );
                    api.control( 'themeworks_accent_color' ).container.find( '.color-picker-hex' )
                    ...
                    api( 'themeworks_options[dark_color]' ).set( colorScheme[value].colors[2] );
                    api.control( 'themeworks_dark_color' ).container.find( '.color-picker-hex' )
    ...
    function updateCSS() {
        var scheme = api( 'themeworks_options[color]' )(), css,
            colors = _.object( colorSchemeKeys, colorScheme[ scheme ].colors );

        // Merge in color scheme overrides.
        _.each( colorSettings, function( setting ) {
            if ( setting == 'themeworks_options[accent_color]' ) {
            colors[ 'accent_color' ] = api( setting )();
            } else if ( setting == 'themeworks_options[dark_color]' ) {
