    ...
    foreach($fields as $field):
        ...
        switch ($field['type']){
            case 'paragraph':
                ...
                $form_output.= '<div class="form-group aol-form-group"><label for="'. $field_key.'">'.$required.sanitize_text_field($field['label']).'</label><div id="'.$field_key.'" class="'.$class.' aol-textbox" style="'.$style.'">'.do_shortcode(wpautop($field['text'])).'</div></div>';
    ...
    foreach ( $keys_order as $key ):
        if ( substr ( $key, 0, 9 ) == '_aol_app_' ){
            ...
            switch ($meta[$key]['type']){
                case 'file':
                    $val = empty($val['file']) ? NULL: '<a href="'.admin_url('?aol_attachment='). aol_crypt($val['file']).'" target="_blank">'.esc_html__('Attachment','ApplyOnline').'</a>';
                    break;
            ...
            $data[] = array('label' => $meta[$key]['label'], 'value' => $val, 'type' => $meta[$key]['type']);
        }
    endforeach;
    return $data;
}
