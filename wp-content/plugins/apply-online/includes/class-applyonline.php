...
class Applyonline {
	...
	public function __construct() {
		...
		$this->define_public_hooks();
                ...
                add_filter( 'views_edit-aol_application', array($this, 'status_filters' ));
 ...
 class Applyonline_AjaxHandler{
     ...
     var $uploads;
        ...
        public function aol_process_app_form(){
            ...
            foreach($form as $key => $val):
                $key = sanitize_key($key);
                    ...
                    if((int)$val['required'] == 1 and $val['type'] != 'file'){
            ...
            $receipents = array();
            foreach($form as $key => $val){
                $app_field = maybe_unserialize($val[0]);
            ...
            if(isset($this->uploads)){
                foreach($this->uploads as $name => $file){
                    $args = array('file'=> FILTER_SANITIZE_URL, 'url' => FILTER_SANITIZE_URL, 'type' => FILTER_SANITIZE_STRING, 'name' => FILTER_SANITIZE_STRING);
            ...
            do_action('aol_before_app_save', $app_data, $_POST);
            ...
            if($pid > 0){
                ...
                do_action('aol_after_app_save', $pid, $app_data);
                ...
                $message = get_option_fixed('aol_application_message', __('Form has been submitted successfully. If required, we will get back to you soon!', 'ApplyOnline'));
        ...
        function applicant_email_notification($post_id, $post, $emails){
            ...
            $subject = sprintf(__('Thank you for your application', 'ApplyOnline'), wp_specialchars_decode($post->post_title));
            ...
            $message=   '<p>'.__('Hi,', 'ApplyOnline').'</p>'
                        .'<p>'
                        .sprintf(__('Thank you for showing your interest in %s advertisement at %s. Your application has been received with id %s. If reqiured we shell get back to you shortly.', 'ApplyOnline'), '<i>'.$post->post_title.'</i>', '<i>'.get_bloginfo('name').'</i>', $post_id)
                        .'</p><p>'
                        .__('Regards,', 'ApplyOnline')."\n"
                        .sprintf(__('Team %s'), get_bloginfo('name'))."\n"
                        .site_url()
                        .'<p>----<br />'
                        .__('Please do not reply to the system generated message.', 'ApplyOnline')
                        .'</p>';

            //$message =  get_option('aol_application_submission_email', $message);
        ...
        function application_email_notification($post_id, $post, $uploads ){
            ...
            if($author_notification){
                ...
                if( !in_array($emails, $author->user_email) ) array_push($emails, $author->user_email);
            }
