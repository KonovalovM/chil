...
class Applyonline_Admin{
	...
	public function __construct( $plugin_name, $version ) {
		...
		$this->version = $version;
                ...
                //Application Print
                add_action('wp_ajax_aol_all_ads', array($this, 'aol_ads_list'));
    ...
    <div class="aol-ad-closing aol-meta">
        ...
        <div class="misc-pub-section curtime misc-pub-curtime">
            <span id="ad-closing">
            <strong><?php echo __('Expires on', 'ApplyOnline'); ?></strong><br />
            </span>
    ...
    class Applyonline_Applications{
        public function __construct(){
            ...
            add_action( 'admin_action_aol_modal_box', array ( $this, 'aol_model_render_action_page') );
        ...
        public function aol_post_row_actions($actions, $post){
            ...
            if($post->post_type == 'aol_application'){
                ...
                $actions['filters'] = '<a rel="permalink" title="'.__('Filter Applications', 'ApplyOnline').'" href="'.  admin_url('edit.php?post_type=aol_application').'&ad='.$post->post_parent.'"><span class="dashicons dashicons-filter"></span></a>';
        ...
        public function aol_application_post_editor ($post){
            ...
            if ( !empty( $post ) and $post->post_type =='aol_application' ):
                ...
                <div class="wrap"><div id="icon-tools" class="icon32"></div>
                    <h3>#<?php echo $post->ID.' - '.$post->post_title; ?></h3>
                        <?php 
                        /*
        ...
        function applications_table_filter(){
            ?>
                &nbsp; <select id="aol-etic-apps-search"><option><?php _e('Filter Applications', 'ApplyOnline'); ?></option></select>
            <?php
        }
                    ...
                    if ( ! empty( $terms ) ) {
                        ...
                        foreach ( $terms as $term ) {
                            ...
                            $out[] = sprintf( 
                                    ...
                                    esc_html( sanitize_term_field( 'name', $status_name, $term->term_id, 'aol_application_status', 'display' ) )
                            );
                        }
                        echo join( ', ', $out );
         ...
         * @return  array
         */
       public function aol_model_render_action_page() {
        ...
        public function applications_filter( $query ) {
            if ( $query->is_main_query() AND is_admin() AND isset($_GET['ad'])) {
                ...
                if(function_exists('pll_get_post_translations')){
                    $post_translation_ids = pll_get_post_translations((int)$_GET['ad'] );
                    $query->set( 'post_parent__in', $post_translation_ids );
                    $query->set( 'orderby', 'title' );
                    $query->set( 'order', 'ASC' );
                    //$query->set( 'post_parent', NULL );
                } else{
                    $query->set( 'post_parent', (int)$_GET['ad'] ); //Set post parent ID.                    
                }
 ...
 class Applyonline_Form_Builder{
	 ...
	 */
        ...
        function ad_features( $post ) {
                ...
                if($keys != NULL):
                    foreach($keys as $key):
                        if(substr($key, 0, 13)=='_aol_feature_'){
                            $features[sanitize_key( substr($key, 13))] = aol_sanitize_array(get_post_meta($post->ID, $key, TRUE));
                        }
                    endforeach;
                endif;
            ...
            <thead>
                <tr>
                    <th class="left"><label for="adfeature_name"><?php esc_html_e('Feature','ApplyOnline');?></label></th>
                    <th><label for="adfeature_value"><?php esc_html_e('Value','ApplyOnline');?></label></th>
                </tr>
            </thead>
            <tbody>
                <tr>
                ...
                <tr>
                    <td colspan="2">
                        <div class=""><div class="button" id="addFeature"><?php esc_html_e('Add Feature','ApplyOnline');?></div></div>
                    </td>
                </tr>
            </tbody>
            </table>
        ...
        public function application_fields_generator($app_fields, $temp = NULL){
            ...
            <div id="<?php echo $adapp_form_generator; ?>" class="field-generator adapp_form_fields <?php echo $adapp_form_generator; ?>" style="display:none;">
                ...
                <div id="aol_new_form" class="aol_form" style="display:none;">
                    ...
                    <div class="aol_text aol_add_field"><label for="adapp_text"><?php _e('Text', 'ApplyOnline') ?></label><textarea class="aol-form-field adapp_text" id="adapp_text" type="text" ></textarea><p class="description"><?php _e('To generate a link, use shortcode [link href="https://google.com" title="My Link Title"]'); ?></p></div>
                    ...
                    <div class="aol_placeholder aol_add_field"><label for="adapp_placeholder"><?php _e('PlaceHolder', 'ApplyOnline') ?></label><input class="aol-form-field adapp_placeholder" type="text" id="adapp_placeholder" ></div>
    <!--            <div class="aol_value aol_add_field"><label><?php _e('Defult value', 'ApplyOnline') ?></label><input type="text" id="adapp_value" placeholder="<?php _e('Defult value') ?>" ></div>-->
                    ...
                    <div class="aol_class"><label for="adapp_class"><?php _e('Classes', 'ApplyOnline') ?></label><input type="text" id="adapp_class" class="aol-form-fild adapp_class" ></div>
        ...
        public function row_popup($key, $val, $template = NULL){ 
            ...
            if(in_array($val['type'], array('checkbox','dropdown','radio','text_area','text','number','email','date','file'))):
                echo '<div><label for="'.$name.'-required">'.__('Required Fieldd', 'ApplyOnline').'</label><input id="'.$name.'-required" class="required_option" type="checkbox" '.$checked.' name="'.$name.'[required]" value="1" /></div>';
            endif;
        ...
        function save_ad( $post_id ){
            ...
            foreach ($_POST as $key => $val):
                ...
                elseif ( substr($key, 0, 9) == '_aol_app_' and isset( $val ) ) {
                    ...
                    update_post_meta( $post_id, $key, $this->array_map_r( 'sanitize_textarea_field', $val ) ); // Add new value.
                    $fields_order[] = $key;
                }
                // 
            endforeach;
...
class Applyonline_Settings extends Applyonline_Form_Builder{
    ...
    public function registers_settings(){
        ...
        register_setting( 'aol_settings_group', 'aol_application_message', array( 'sanitize_callback' => 'sanitize_textarea_field') );
            ...
            <form action="options.php" method="post" name="">
                ...
                ?>
                    ...
                    <tr>
                        ...
                        <td>
                            <label class="switch">
                                <input type="checkbox" name="aol_multistep" <?php echo get_option('aol_multistep') ? 'checked="checked"':Null; ?> >
                    ...
                    <tr>
                        ...
                        <td>
                            <label class="switch">
                                <input type="checkbox" name="aol_nonce_is_active" <?php echo get_option('aol_nonce_is_active', 1) ? 'checked="checked"':Null; ?> >
                                <span class="slider"></span>
                             </label>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <p class="description"><?php _e('Default: Read More. It appears in each ad on ads listing page.', 'ApplyOnline'); ?></p>
                        </td>
                    </tr>
                    <tr>
