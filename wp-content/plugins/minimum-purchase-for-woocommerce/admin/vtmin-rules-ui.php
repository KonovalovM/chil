...
class VTMIN_Rules_UI { 
  ...
  public function vtmin_enqueue_script() {
    ...
    if( 'vtmin-rule' == $post_type ){ 
        wp_register_style( 'vtmin-admin-style', VTMIN_URL.'/admin/css/vtmin-admin-style-v003.css' );  //v1.09.6 ==> new version for new metabox
        ...
        //REMOVE THIS WHEN INCLUDE/EXCLUDE SELECT2 LOGIC INTRODUCED!!
        //AJAX resources
  ...
  public  function vtmin_add_metaboxes() {
      ...
      add_meta_box('vtmin-rule-pop-must-be-in-cart', __('Minimum Quantity Required for Any Cart Purchase', 'vtmin'), array(&$this, 'vtmin_pop_must_be_in_cart'), 'vtmin-rule', 'normal', 'default');  //v1.09.91.2 
  ...
  //v1.08 end
   ...
   //v1.09.91.2 NEW FUNCTION                                                                        
    public    function vtmin_pop_must_be_in_cart() {
        global $post, $vtmin_info, $vtmin_rule, $vtmin_rules_set; 
        //error_log( print_r(  'function vtmin_pop_must_be_in_cart ' , true ) );
          ?>
        <div class="rule_message clear-left" id="cust-msg-text-area">
           <span class="newColumn1" id=cust-msg-text-label-area>
              <h3 style="color: #aaa;"><?php _e('Minimum Quantity from this Rule Required for ANY Purchase', 'vtmin')?></h3> 
              <span id='cust-msg-optional'><?php _e('(optional, requires "Custom Message Text" below if used)', 'vtmin')  ?></span>
           </span>   
           
          <label id="rule-pop-must-be-in-cart_label">                                               
              <input id="rule-pop-must-be-in-cart"  type="checkbox" value="yes" name="rule-pop-must-be-in-cart" disabled="disabled" 
                <? if ($vtmin_rule->rule_pop_must_be_in_cart == "yes") {echo 'checked="checked"';}?>
              >                                                                                                                     
              <span class="text-float" style="color: #aaa;"><?php _e('Require this Rule Minimum Quantity for ANY Cart Purchases', 'vtmin')?>  
              </span>&nbsp;&nbsp;                                         
          </label>           
